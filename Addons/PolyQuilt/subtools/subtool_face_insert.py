...
class SubToolFaceInsert(SubTool) :
    ...
    def OnDraw3D( self , context  ) :
        ...
        if self.is_hit_center :
            lines = ( ( self.center , l2w( v.co )) for v in self.hit_face.element.verts )
...
#   add_v3_v3v3(ab, a, b);            
 ...
 #           return (UNLIKELY(angle_cos < SMALL_NUMBER)) ? 1.0f : (1.0f / angle_cos);
            ...
            lines = ( ( l2w( l.vert.co ) ,nor(l))  for l in self.hit_face.element.loops )
            draw_util.draw_lines3D( bpy.context, verts = sum( lines , () )  , color= self.color_split() , primitiveType='LINES' )

            lines = ( ( nor(l) , nor(l.link_loop_next) ) for l in self.hit_face.element.loops )
            draw_util.draw_lines3D( bpy.context, verts = sum( lines , () )  , color= self.color_split() , primitiveType='LINES' )
    ...
    def DoInsert( self , face ) :
        if self.is_hit_center :
            bmesh.ops.poke( self.bmo.bm, faces  = [face] , offset  = 0 , center_mode = 'MEAN' , use_relative_offset = False )
            self.bmo.UpdateMesh()
        elif self.extrude_dst > 0:
#            bmesh.ops.inset_individual(self.bmo.bm, faces  = [face] , thickness = self.extrude_dst , use_even_offset = True )
            #(bm, faces, thickness, depth, use_even_offset, use_interpolate, use_relative_offset)
            bmesh.ops.inset_region(self.bmo.bm, faces  = [face] , thickness = self.extrude_dst , use_even_offset = True)
